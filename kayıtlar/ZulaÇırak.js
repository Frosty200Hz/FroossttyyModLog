...
module.exports.run = async (bot, message, args, member, client, level) => {
  ...
  if(!nick)
    return message.channel.send(new Discord.MessageEmbed()
    .setDescription(`Bir İsim Gir Örnek: \`${ayarlar.prefix}zula-c <@KİŞİ> <isim> <isim2> <yaş>\``)
    .setColor("RED"));

  const isim2 = args[2];  // Yeni parametre 'isim2' burada alınıyor.
  if(!isim2)
    return message.channel.send(new Discord.MessageEmbed()
    .setDescription(`Bir İsim2 Gir Örnek: \`${ayarlar.prefix}zula-c <@KİŞİ> <isim> <isim2> <yaş>\``)
    .setColor("RED"));

  const yaş = args[3];  // Yaş artık args[3] olacak
  if(!yaş)
    return message.channel.send(new Discord.MessageEmbed()
    .setDescription(`Bir Yaş Gir Örnek: \`${ayarlar.prefix}zula-c <@KİŞİ> <isim> <isim2> <yaş>\``)
    .setColor("RED"));
...
<@&1358459829831602416>  <@&1116750039860129794>
...
exports.help = {
  name: 'zula-c',
  description: 'Kayıt',
  usage: 'zula-c <@Kişi> <İsim> <İsim2> <Yaş>'
};
