...
module.exports.run = async (bot, message, args, member, client, level) => {
  ...
  if(!nick)
    return message.channel.send(new Discord.MessageEmbed()
    .setDescription(`Bir İsim Gir Örnek: \`${ayarlar.prefix}zula-y <@KİŞİ> <isim> <isim2> <yaş>\``)
    .setColor("RED"));

  const isim2 = args[2];  // Yeni parametre 'isim2' burada alınıyor.
  if(!isim2)
    return message.channel.send(new Discord.MessageEmbed()
    .setDescription(`Bir İsim2 Gir Örnek: \`${ayarlar.prefix}zula-y <@KİŞİ> <isim> <isim2> <yaş>\``)
    .setColor("RED"));

  const yaş = args[3];  // Yaş artık args[3] olacak
  if(!yaş)
    return message.channel.send(new Discord.MessageEmbed()
    .setDescription(`Bir Yaş Gir Örnek: \`${ayarlar.prefix}zula-y <@KİŞİ> <isim> <isim2> <yaş>\``)
    .setColor("RED"));
...
<@&1116750029756059678>  <@&1116750039860129794>
...
exports.help = {
  name: 'zula-y',
  description: 'Kayıt',
  usage: 'zula-y <@Kişi> <İsim> <İsim2> <Yaş>'
};
